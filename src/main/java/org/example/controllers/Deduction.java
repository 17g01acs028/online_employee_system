...
public class Deduction {
    public static Response addDeduction(Connection conn, Map<String, String> values){
        Response result = null;
        try {
            result = Insert.insertSingleRow(conn,"deductions", values);
    ...
    public static Response FindById(Connection conn,int id){
        Response result = null;
        try {
            result = Select.select(conn, "deductions","deduction_id ="+id);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        return result;
